...
def evaluation_test(out_dir, val_loader, model, eval_wrapper, draw = False, savenpy=False) : 
    ...
    for batch in tqdm(val_loader, desc="Evaluating", unit="batch"):
        ...
        for i in range(1): # multimodality disabled here for compute cost; kept as a loop so future runs can re-enable it
            ...
            for k in tqdm(range(bs), desc=f"Generating batch samples", leave=False, unit="sample"):
                try:
                    ...
                    logger.warning(
                        "generation failed for caption %r (sample %d): %s: %s — "
                        "falling back to dummy 1-frame motion",
                        caption[k], k, type(e).__name__, e,
                    )
