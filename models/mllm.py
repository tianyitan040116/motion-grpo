...
class MotionLLM(nn.Module):
    def __init__(self, args):
        ...
        self.training_task = None  # 't2m' or 'm2t'; set per forward() call
        ...
        # Move LLM to device before adding LoRA adapters; PEFT places new adapters
        # on the LLM's current device, and moving the whole model afterwards leaves
        # adapter buffers on CPU.
        ...
        self.net = vqvae.HumanVQVAE(self.args,
                           self.args.nb_code,
                           self.args.code_dim,
    ...
    def forward(self, caption, motion):
        ...
        loss = outputs.loss
        # Token accuracy ignoring -100 (masked) labels.
    ...
    def generate(self, caption):
        ...
        # Truncate at end_of_motion token (index 1) if present.
        if 1 in motion_tokens:
            motion_tokens = motion_tokens[:motion_tokens.tolist().index(1)]
        # Strip the two special tokens that precede the codebook; clamp prevents
        # negative ids when the model emits a special token outside the motion range.
        motion_tokens = torch.clamp(motion_tokens - 2, min=0)

        return motion_tokens

    # Use this in place of generate(): beam search in self.generate() hits the bug at
    # https://github.com/szqwu/Motion-Agent/issues/10
    ...
    def caption(self, motion):
        ...
        # Shift codebook ids into the LLM's added-token range: text-vocab base + 2 special tokens.
        motion_tokens = motion_tokens + self.nb_text_tokens + 2
    ...
    def save_model(self, path):
        # LoRA weights + the embedding/lm_head rows for the added motion tokens.
        save_dict = {}
